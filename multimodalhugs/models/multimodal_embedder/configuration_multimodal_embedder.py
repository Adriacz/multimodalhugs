...
class MultiModalEmbedderConfig(PretrainedConfig):
    ...
    feat_dim : int
        Feature extractor output dimension for the video modality. If offline features are used, the
        dimensionality of the extracted features.
    feature_extractor_type : str, optional
        Type of feature extractor to use for the video modality.
    feature_extractor_config : dict, optional
        Hyperparameters for the video feature extractor. Parameters not specified
    ...
    pretrained_feature_extractor : str, optional
        Path or identifier for a pretrained video feature extractor.
    freeze_feature_extractor : bool
        Whether to freeze video feature extractor parameters during training.
    multimodal_mapper_type : str, optional
        Type of video multimodal mapper: one of {"linear", "adapter", "cnn_adapter"}.
    multimodal_mapper_layer_norm_before : bool
        Whether to apply LayerNorm before the video mapper.
    multimodal_mapper_layer_norm : bool
        Whether to apply LayerNorm inside the video mapper.
    multimodal_mapper_activation : bool
        Whether to apply ReLU at the video mapper output.
    multimodal_mapper_factor : int, optional
        Overparameterization factor used by the adapter-based video mapper.
    multimodal_mapper_dropout : float, optional
        Dropout probability for the video multimodal mapper.
    adapter_ksize : tuple of int, optional
        Kernel size(s) for the cnn_adapter video mapper.
    adapter_stride : tuple of int, optional
        Stride(s) for the cnn_adapter video mapper.
    freeze_multimodal_mapper : bool
        Whether to freeze video multimodal mapper parameters during training.
    audio_feat_dim : int, optional
        Feature extractor output dimension for the audio modality.
    audio_feature_extractor_type : str, optional
        Type of feature extractor to use for the audio modality.
    audio_feature_extractor_config : dict, optional
        Hyperparameters for the audio feature extractor.
    audio_pretrained_feature_extractor : str, optional
        Path or identifier for a pretrained audio feature extractor.
    audio_freeze_feature_extractor : bool
        Whether to freeze audio feature extractor parameters during training.
    audio_multimodal_mapper_type : str, optional
        Type of audio multimodal mapper: one of {"linear", "adapter", "cnn_adapter"}.
    audio_multimodal_mapper_layer_norm_before : bool
        Whether to apply LayerNorm before the audio mapper.
    audio_multimodal_mapper_layer_norm : bool
        Whether to apply LayerNorm inside the audio mapper.
    audio_multimodal_mapper_activation : bool
        Whether to apply ReLU at the audio mapper output.
    audio_multimodal_mapper_factor : int, optional
        Overparameterization factor used by the adapter-based audio mapper.
    audio_multimodal_mapper_dropout : float, optional
        Dropout probability for the audio multimodal mapper.
    audio_freeze_multimodal_mapper : bool
        Whether to freeze audio multimodal mapper parameters during training.
    ...
    def __init__(
        ...
        audio_feature_extractor_config: Optional[Dict[str, Any]] = None,
        audio_pretrained_feature_extractor: Optional[str] = None,
        audio_freeze_feature_extractor: bool = False,
        ...
        **kwargs):
        ...
        self.audio_feature_extractor_config = audio_feature_extractor_config
        ...
        # Video feature extractor config
        if self.feature_extractor_type is not None:
            feature_extractor_config_class = get_feature_extractor_class(self.feature_extractor_type)[1]
            self.feature_extractor_config = feature_extractor_config_class(
                **(self.feature_extractor_config if self.feature_extractor_config is not None else {})
            )
        else:
            self.feature_extractor_config = None

        # Audio feature extractor config
        if self.audio_feature_extractor_type is not None:
            audio_feature_extractor_config_class = get_feature_extractor_class(self.audio_feature_extractor_type)[1]
            self.audio_feature_extractor_config = audio_feature_extractor_config_class(
                **(self.audio_feature_extractor_config if self.audio_feature_extractor_config is not None else {})
            )
        else:
            self.audio_feature_extractor_config = None
